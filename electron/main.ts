...
app.whenReady().then(async () => {
  electronApp.setAppUserModelId('com.genfinds.app')
...
ipcMain.handle(
  'conta:update',
  async (_, id: number, usuarioId: number, updates: Partial<Conta>) => {
    try {
      // Validação de entrada
      const idValidation = validateData(IdSchema, id)
      if (!idValidation.success) {
...
ipcMain.handle(
  ...
  async (_, categoria: Omit<Categoria, 'id' | 'created_at' | 'updated_at'>) => {
    try {
      ...
      const data = db.createCategoria(
        validation.data as Omit<Categoria, 'id' | 'created_at' | 'updated_at'>
      )
      return { success: true, data }
    } catch (error) {
...
ipcMain.handle(
  'categoria:update',
  async (_, id: number, usuarioId: number, updates: Partial<Categoria>) => {
    try {
      // Validação de entrada
      const idValidation = validateData(IdSchema, id)
      if (!idValidation.success) {
...
ipcMain.handle(
  'orcamento:update',
  async (_, id: number, usuarioId: number, updates: Partial<Orcamento>) => {
    try {
      const idValidation = validateData(IdSchema, id)
      if (!idValidation.success) {
        return { success: false, error: idValidation.error }
...
ipcMain.handle(
  'cartao:update',
  async (_, id: number, usuarioId: number, updates: Partial<Cartao>) => {
    try {
      const idValidation = validateData(IdSchema, id)
      if (!idValidation.success) {
        return { success: false, error: idValidation.error }
...
ipcMain.handle(
  'parcela:update',
  async (_, id: number, usuarioId: number, updates: Partial<Parcela>) => {
    try {
      const idValidation = validateData(IdSchema, id)
      if (!idValidation.success) {
        return { success: false, error: idValidation.error }
...
ipcMain.handle(
  'transacao:update',
  async (_, id: number, usuarioId: number, updates: Partial<Transacao>) => {
    try {
      const idValidation = validateData(IdSchema, id)
      if (!idValidation.success) {
        return { success: false, error: idValidation.error }
