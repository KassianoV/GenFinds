...
const ImportManager = {
    ...
    categorizationRules: {
        // Alimentação
        'alimentacao': ['mercado', 'supermercado', 'padaria', 'restaurante', 'lanche', 'ifood', 'uber eats', 'rappi', 'mc donald', 'burger king', 'subway', 'pizza'],
        
        // Transporte
        'transporte': ['uber', '99', 'taxi', 'combustivel', 'gasolina', 'posto', 'estacionamento', 'pedágio', 'metrô', 'onibus'],
        
        // Saúde
        'saude': ['farmacia', 'drogaria', 'hospital', 'clinica', 'medico', 'laboratorio', 'exame', 'consulta', 'plano de saude'],
        
        // Lazer/Entretenimento
        'lazer': ['cinema', 'netflix', 'spotify', 'amazon prime', 'bar', 'balada', 'show', 'teatro', 'parque', 'viagem'],
        
        // Casa/Moradia
        'moradia': ['aluguel', 'condominio', 'agua', 'luz', 'energia', 'gas', 'internet', 'telefone', 'limpeza', 'manutencao'],
    ...
    attachEventListeners() {
        ...
        fileUploadArea.addEventListener('click', () => {
            if (!document.getElementById('fileInfo').style.display || 
    ...
    async processImport() {
        ...
        try {
            ...
            console.log(`✅ ${categorizados} de ${this.parsedData.length} transações categorizadas automaticamente`);
        ...
        while ((match = stmttrnRegex.exec(content)) !== null) {
            ...
            if (dateMatch && amountMatch) {
                ...
                const description = (memoMatch ? memoMatch[1] : '') || 
                                  (nameMatch ? nameMatch[1] : 'Sem descrição');
        ...
        for (const [categoriaChave, keywords] of Object.entries(this.categorizationRules)) {
            for (const keyword of keywords) {
                if (desc.includes(keyword.toLowerCase())) {
                    ...
                    let categoriaObj = AppState.categorias.find(c => 
                        c.nome.toLowerCase() === categoriaChave && c.tipo === tipo
                    );
                    
                    // Se não encontrar, tenta match parcial
                    if (!categoriaObj) {
                        categoriaObj = AppState.categorias.find(c => 
                            c.nome.toLowerCase().includes(categoriaChave) && c.tipo === tipo
                        );
                    }
                    ...
                    if (!categoriaObj) {
                        categoriaObj = AppState.categorias.find(c => 
                            categoriaChave.includes(c.nome.toLowerCase()) && c.tipo === tipo
                        );
                    }
    ...
    async confirmImport() {
        ...
        try {
            ...
            for (let i = 0; i < this.parsedData.length; i++) {
                ...
                if (!transacao.categoria_id) {
                    ...
                    if (categoriaDefault) {
                        ...
                        console.log(`📌 Usando categoria padrão "${categoriaDefault.nome}" para: ${item.descricao}`);
                    }
                }
            ...
            if (ignorados > 0) {
                Utils.showWarning(`${ignorados} transações foram ignoradas por falta de categoria do tipo correspondente. Crie categorias de Receita/Despesa e tente novamente.`);
            }
