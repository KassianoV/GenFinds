            ...
            try {
                ...
                for (let i = 1; i < linhas.length; i++) {
                    ...
                    try {
                        const transacao = {
                            data: colunas[0],
                            descricao: colunas[1].replace(/"/g, ''),
                            valor: parseFloat(colunas[5]),
                            tipo: colunas[4],
                            usuario_id: AppState.currentUser.id,
                            observacoes: colunas[6]?.replace(/"/g, '')
                        };

                        const categoriaNome = colunas[2].replace(/"/g, '');
                        const categoria = AppState.categorias.find(c => c.nome === categoriaNome);
                        if (categoria) {
                        ...
                        const contaNome = colunas[3].replace(/"/g, '');
                        const conta = AppState.contas.find(c => c.nome === contaNome);
                        if (conta) {
                ...
                Utils.showSuccess(`Importadas: ${importadas} | Erros: ${erros}`);
            } catch (error) {
                console.error('Erro ao importar:', error);
                Utils.showError('Erro ao importar arquivo');
            }
        };
